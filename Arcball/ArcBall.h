...
//Math functions
    ...
    static void Matrix3fSetRotationFromQuat4f(Matrix3fT* NewObj, const Quat4fT* q1)
    {
		...
		float z = q1->s.Z;
		float w = q1->s.W;
		NewObj->s.XX = 1 - 2 * y*y - 2 * z*z; NewObj->s.YX = 2 * x*y + 2 * w*z; NewObj->s.ZX = 2 * x*z - 2 * w*y;
		NewObj->s.XY = 2*x*y-2*w*z; NewObj->s.YY = 1-2*x*x-2*z*z; NewObj->s.ZY = 2*y*z+2*w*x;
		NewObj->s.XZ = 2*x*z+2*w*y; NewObj->s.YZ = 2*y*z-2*w*x; NewObj->s.ZZ = 1-2*x*x-2*y*y;
	}

    inline
		static Matrix3fT Matrix3fMulMatrix3f(const Matrix3fT* NewObj, const Matrix3fT* m1)
    {
